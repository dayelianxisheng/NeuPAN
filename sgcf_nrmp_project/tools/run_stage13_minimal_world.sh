...
out="$repo/sgcf_nrmp_project/artifacts/stages/stage_13_minimal_gazebo_sensor_world"
gz_image=$(docker image inspect sgcf-gazebo-harmonic:hlms-media-fix --format '{{.Id}}')
bridge_image=$(docker image inspect sgcf-ros2-humble-gzharmonic-bridge:local --format '{{.Id}}')
mkdir -p "$out/logs" "$out/runtime"
current_gz=""; current_bridge=""

cleanup_round() {
  local gz_name=$1 bridge_name=$2
  docker stop -t 10 "$bridge_name" "$gz_name" >/dev/null 2>&1 || true
  docker rm "$bridge_name" "$gz_name" >/dev/null 2>&1 || true
}
cleanup_active() { [[ -z "$current_gz" ]] || cleanup_round "$current_gz" "$current_bridge"; }
trap cleanup_active EXIT

for mode in zero motion; do
  gz_name="sgcf_gz_stage13_${mode}"; bridge_name="sgcf_bridge_stage13_${mode}"
  current_gz="$gz_name"; current_bridge="$bridge_name"
  partition="sgcf_stage13_${mode}"; domain=$([[ $mode == zero ]] && echo 73 || echo 74)
  log="$out/logs/$mode"; runtime="$out/runtime/$mode"; mkdir -p "$log" "$runtime"
  cleanup_round "$gz_name" "$bridge_name"
  docker run -d --name "$gz_name" --network host -e GZ_PARTITION="$partition" -v "$repo:/workspace" -w /workspace "$gz_image" bash -lc 'exec gz sim -s -r --headless-rendering --render-engine-server ogre2 -v 4 /workspace/sgcf_nrmp_project/gazebo/worlds/single_static_obstacle.sdf' >"$log/gazebo_container_id.txt"
  ready=false
  ...
  set +e
  docker exec -e ROS_DOMAIN_ID="$domain" -e STAGE13_MODE="$mode" -e STAGE13_OUT="/workspace/sgcf_nrmp_project/artifacts/stages/stage_13_minimal_gazebo_sensor_world/runtime/$mode" "$bridge_name" bash -lc 'source /opt/ros/humble/setup.bash; export PYTHONPATH=/workspace/sgcf_nrmp_project/tools:${PYTHONPATH:-}; exec python3 /workspace/sgcf_nrmp_project/tools/stage13_sensor_command_audit.py' >"$log/audit_stdout.txt" 2>"$log/audit_stderr.txt"
  status=$?
  set -e
  kill -TERM "$capture" 2>/dev/null || true; wait "$capture" 2>/dev/null || true
  docker exec "$bridge_name" bash -lc 'source /opt/ros/humble/setup.bash; ros2 topic list -t; ros2 node list' >"$log/final_ros_graph.txt" 2>&1 || true
...
docker ps -a --format '{{.Names}}' | grep '^sgcf_.*stage13_' >"$out/logs/residual_containers.txt" || true
pgrep -af 'stage13_sensor_command_audit|gz sim.*single_static_obstacle|parameter_bridge' >"$out/logs/residual_processes.txt" || true
