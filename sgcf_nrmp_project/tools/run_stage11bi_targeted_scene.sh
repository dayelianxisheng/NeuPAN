...
cleanup() {
  ...
  if [[ -n "$server_pid" ]]; then
    set +e; wait "$server_pid" 2>/dev/null; server_exit=$?; set -e
  fi
...
python3 -c 'import json,sys; json.dump({"ready":True,"scene":sys.argv[1],"world":sys.argv[1],"lidar_requested":20,"camera_requested":5,"odometry_requested":20,"clock_requested":20},open(sys.argv[2],"w"),indent=2)' "$scene" "$out/runtime.json"
