...
for scene in "${scenes[@]}"; do
  ...
  docker exec "$ros_name" bash -lc \
    "source /opt/ros/humble/setup.bash; export CUDA_VISIBLE_DEVICES=''; export NVIDIA_VISIBLE_DEVICES=void; export PYTHONPATH=/workspace/sgcf_nrmp_project/core/src:/workspace/sgcf_nrmp_project/ros2_ws/src/sgcf_nrmp_bridge:\${PYTHONPATH:-}; export STAGE11CC_SCENE=${scene}; export STAGE11CC_MODES=${mode_list}; export STAGE11CC_SCENE_OUT=/workspace/sgcf_nrmp_project/artifacts/stages/${stage_dir}/runtime/${run_id}; export STAGE11CC_REPO=/workspace; export STAGE15_ORACLE_MAP_JSON='${STAGE15_ORACLE_MAP_JSON:-}'; export STAGE15_SCENE_LABEL='${STAGE15_SCENE_LABEL:-}'; export STAGE15B_EVALUATION_DURATION_S='${STAGE15B_EVALUATION_DURATION_S:-0}'; export STAGE15B_GOAL_TOLERANCE_M='${STAGE15B_GOAL_TOLERANCE_M:-0.25}'; export STAGE15B_REFERENCE_WAYPOINTS_JSON='${STAGE15B_REFERENCE_WAYPOINTS_JSON:-}'; export STAGE15B_WALL_TIMEOUT_S='${STAGE15B_WALL_TIMEOUT_S:-90}'; exec /opt/sgcf_planner_venv/bin/python -m sgcf_nrmp_bridge.stage11cc_planner_shadow_node" \
